...
struct WLNParser {
    ...
    {   
        if (src && dst){
          ...
          fprintf(stderr,"ERROR: fusing null pointers\n");
    }
    ...
    int perifused_ring(std::vector<OpenBabel::OBAtom*> &ring,std::vector<char> char_vector, std::vector<int> atom_vector,std::vector<char> peri_vector, int ring_positions) {
        ...
        if (ring.empty() || char_vector.empty())
            return error();
                            ...
                            if (ptr[2]>='A' && ptr[2]<='Z' &&
                                ...
                                if (order==1 || order==2) {
                                    ...
                                    if (spiro){
                                        ...
                                        if (*ptr == '\0' || rings.empty())
                                          return error();
                              
                                        if (rings.back().size() <= loc+1 || rings.back().size() <= spiro_pos-1)
                                          return error();

                                        std::cout << rings.back().size() << std::endl;

                                        fuse(prev,rings.back()[loc+1],order); // "Surround" the spiro center
                                        fuse(prev,rings.back()[spiro_pos-1],order);
                                        if(!mol->DeleteAtom(rings.back()[loc]),false)
                                          return error();
...
bool MBWLNMurckoScaffold(const char *ptr, OpenBabel::OBMol* mol)
{
    if (ptr[0] == 'L' || ptr[0] == 'T'){
        ...
        fprintf(stderr,"Error: WLN must be in Scaffold Notation\n");
        return false;}
}

std::string  MBStringDecomp(const char *ptr){
    std::string copy = std::string(ptr);
    // This should be doable in two passes.
    copy.insert (0, 1, '\0');       // Pre-Null inserted to fix indexing the loc-1 pos.
    copy.insert (copy.size(), 1, '\0');  // Post-Null inserted to fix indexing the loc+1 pos.
    unsigned int ring_start=0;
    unsigned int ring_end=0;
    unsigned int size = copy.size();
    unsigned int i = 0;
    while (i <= size){ // Parse 1 for Rings
        if ((copy[i] == 'L' || copy[i] == 'T') && copy[i-1] == '\0')
            ring_start = i;
        if ((copy[i] >= 'A' && copy[i] <= 'Z') && (copy[i+1] == 'L' || copy[i+1] == 'T')){
            if (ring_start == 0)
                ring_start = i;}
        ...
        if (copy[i] == 'J' && (copy[i+1]==' ' || copy[i+1]=='\0'))
            ring_end = i;
        if (ring_start>0 && ring_end>0){
            copy.erase(ring_start, ring_end-ring_start+1);
            size = copy.size();
            ring_start= 0;
            ring_end = 0;
            i=0;}
    ...
    i=0;
    size = copy.size();
    copy.erase(remove(copy.begin(), copy.end(), '\0'), copy.end());
    unsigned int stored = 0;
    while (i<=size){ // Parse 2 for Groups + Extender Removal (-)
        if (copy[i] == ' ' && (copy[i+1] >= 'A' && copy[i+1]<='Z')) {
            copy[i] = '&';
            stored = i;
            copy[i+1] = '#';
            copy.insert(i," ");
        ...
        if (copy[i] =='-'){
            copy.erase(stored, i-stored+1 +1); // space created from previous process
            size = copy.size();
            i = 0;}
        i++;}

    if (!copy.empty())
      copy.erase(0,2); // How this is set up I think the first character will always be a space!
    return copy;              // Plus a starting '&' character causes errors so 2 index needed.
}

bool MBWLNRGroupDecomp(const char *ptr, OpenBabel::OBMol* mol){
    if (ptr[0] == 'L' || ptr[0] == 'T'){
        std::string copy = MBStringDecomp(ptr);
        const char *edit = copy.c_str();
        WLNParser wp(edit,mol);
        int result = wp.parse_inorganic();
        if (result == 0) {
            if (!wp.parse())
                return false;
...
bool MBWLNGraphScaffold(const char *ptr, OpenBabel::OBMol* mol){
    ...
    if(MBWLNMurckoScaffold(ptr, mol)){
    OpenBabel::OBAtom *atom;
    OpenBabel::OBBond *bond;
    std::vector<OpenBabel::OBAtom*>::iterator atom_iter;

    for (atom = mol->BeginAtom(atom_iter); atom; atom = mol->NextAtom(atom_iter)) {
        atom->SetAtomicNum(6);
        ...
        if (valence !=4){
            atom->SetImplicitHCount(hcount + (4-valence));
        }
    }
    return true;}
    else{
...
const char*  MurckoScaffold(const char *test_string, const char *format)
{
    ...
    conv.SetOutFormat(format);
    if(MBWLNMurckoScaffold(test_string,wlnMol)){
        std::string res = conv.WriteString(wlnMol, true);
...
const char*  GraphScaffold(const char *test_string, const char *format)
{
    ...
    conv.SetOutStream(&std::cout);
    conv.SetOutFormat(format);
    if(MBWLNGraphScaffold(test_string,wlnMol)){
        std::string res = conv.WriteString(wlnMol, true);
...
const char*  RGroupDecomp(const char *test_string, const char *format)
{
    ...
    conv.SetOutStream(&std::cout);
    conv.SetOutFormat(format);
    if(MBWLNRGroupDecomp(test_string,wlnMol)){
        std::string res = conv.WriteString(wlnMol, true);
